...
felix.auto.start.<xsl:value-of select="$i"/>=file:apache-cxf-dosgi-ri-1.1-SNAPSHOT/dosgi_bundles/<xsl:value-of select="substring-after(text(), '.dir/apache-cxf-dosgi-ri-1.1-SNAPSHOT/dosgi_bundles/')"/>
    </xsl:for-each>
  </xsl:template>
</xsl:transform>
